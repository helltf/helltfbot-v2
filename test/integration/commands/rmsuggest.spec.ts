...
fdescribe('test rmsuggest command', () => {
    ...
    it('required id is undefined return error message', async() => {
        ...
        expect(response.response).toBe('You need to specify an id to delete your suggestion')
    ...
    it('id is defined but not in database return error', async() => {
        ...
        expect(response.success).toBeFalse()
        expect(response.response).toBe(`Id ${id} not existing or the suggestion is created by somebody else`)
    })
    ...
    it('id is defined and existing in the database, delete the entry',async () => {
        ...
        expect(response.response).toBe(`Succesfully removed your suggestion with id ${id}`)
